# =============================================
# Federal Funds Rate vs 90-Day Commercial Paper Rate (Final)
# =============================================
...
commercial_paper <- fredr(
  ...
  mutate(series = "90-Day Commercial Paper Rate")

# 5. Combine data
combined_data <- bind_rows(fed_data, commercial_paper) %>% 
...
ggplot() +
  ...
  labs(
    title = "Federal Funds Rate vs. 90-Day Commercial Paper Rate (1971-Present)",
    subtitle = "Gray shading = NBER recessions | Dotted lines = 5-year intervals",
    caption = paste("Sources: FRED (DCPF3M, FEDFUNDS) |", format(Sys.Date(), "%Y-%m-%d")),
    x = NULL,
    y = "Interest Rate (%)"
  ) +
...
# Save the plot
ggsave(
  "fed_vs_commercial_paper_final.png",
  width = 16,
  height = 9,
  dpi = 300,
  bg = "white"
